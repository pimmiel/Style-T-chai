...
export default async function LikesPage() {
  ...
  if (postIds.length > 0) {
    ...
    posts = (data ?? []).map((row: any) => {
      ...
      return {
        ...
        _liked: likeMap[row.id]?.liked ?? true,
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      <div className="mb-8">
        <h1 className="text-2xl font-semibold tracking-tight">{t.likes.title}</h1>
        <p className="text-muted-foreground mt-1">{t.likes.subtitle}</p>
      </div>

      {posts.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-muted-foreground gap-3">
          <Heart className="w-10 h-10 opacity-30" />
          <p className="text-sm">{t.likes.empty}</p>
        </div>
      ) : (
